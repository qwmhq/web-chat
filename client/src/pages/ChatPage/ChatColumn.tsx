...
const ChatColumn = ({ className }: Props) => {
            ...
            <div className="px-4 overflow-y-auto flex flex-col gap-1.5">
              {activeChat.messages
                .map((m, idx) => (
                  <div
                    key={idx}
                    className={`w-fit max-w-[70%] px-4 py-2 rounded-2xl ${m.senderId === userState.currentUser?.id ? "mr-0 ml-auto bg-primary-foreground text-primary" : "bg-primary text-primary-foreground"}`}
                  >
                    {m.text}
                  </div>
                ))
